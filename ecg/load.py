...
class Preproc:
    ...
    def __init__(self, ecg, labels):
        ...
        self.classes = sorted(set(l for label in labels for i in label))
        self.int_to_class = dict(list(zip(list(range(len(self.classes))),
                                 self.classes)))
    ...
    def process_y(self, y):
        ...
        y = pad([[self.class_to_int[c] for c in s] for s in y], val=3,
                dtype=np.int32)
...
def load_dataset(data_json):
    with open(data_json, 'r') as fid:
        data = [json.loads(l) for i in fid]
    labels = []
    ecgs = []
